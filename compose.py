...
def check_overlapping_names(
    ...
    def _edge_names(
        graph, exclude=set()):  # type: (GraphProto, Set[Text]) -> List[Text]
        edges = []
        for n in graph.node:
            for i in n.input:
    ...
    overlap = _overlapping(_edge_names(g1),
                           _edge_names(g2, exclude=io_map_inputs))
    if len(overlap) > 0:
        result.append(('edge', overlap))

    overlap = _overlapping([e.name for e in g1.value_info],
                           [e.name for e in g2.value_info])
    if len(overlap) > 0:
        result.append(('value_info', overlap))

    overlap = _overlapping([e.name for e in g1.initializer],
                           [e.name for e in g2.initializer])
    if len(overlap) > 0:
